...
from bucket.policy import read_bucket_policy, assign_policy, lifecycle
from object.crud import download_file_and_upload_to_s3, get_objects, count_extention, delete_objects, multipart_upload
...
parser.add_argument("-mp",
                    "--multipart_upload",
                    choices=["False", "True"],
                    help="upload large files to bucket",
                    type=str,
                    nargs="?",
                    const="True",
                    default="False")
...
parser.add_argument("-lc",
                    "--lifecycle",
                    type=str,
                    help="object will delete after 120 days",
                    default=None)
...
parser.add_argument("-cev",
                    "--count_extention_value",
                    type=str,
                    help="counts_extention",
                    nargs="?",
                    const="True",
                    default="False")
...
def main():
  ...
  if args.bucket_name:
    ...
    if args.count_extention_value == "True":
      counter = {}
      for each in count_extention(s3_client, args.bucket_name)['Contents']:
        print(each['Key'].split(".")[-1])
        if not counter.get(each['Key'].split(".")[-1]):
          counter[each['Key'].split(".")[-1]] = 1
        else:
          counter[each['Key'].split(".")
                  [-1]] = counter[each['Key'].split(".")[-1]] + 1
      print(counter)

    if args.delete_bucket == "True" and delete_bucket(s3_client,
                                                      args.bucket_name):
      print(f"{args.bucket_name} successfully  deleted")

    if args.lifecycle == "True":
      print(f'Lifecycle policy added to {args.bucket_name} successfully')
